...
func _init() -> void:
	...
	shell.handle_player_action(KEYBOARD_SPACE, false)
	var lacewood_evidence: Dictionary = shell.presentation_evidence()
	...
	test.expect(
		...
		and lacewood_evidence.get("birthday_stars") == ["birthday-star.lacewood"]
		and lacewood_evidence.get("rainbow_paths") == ["rainbow-path.lacewood"]
		and lacewood_evidence.get("playful_bumps") == 3
		and lacewood_evidence.get("cloud_rests") == 1
		and lacewood_evidence.get("flight_control_cycles") == 3,
...
func _fly_through_pellegrino_peak(shell: StorybookShell) -> void:
	...
	test.expect(
		shell.presentation_evidence().get("state") == "birthday_star_moment",
		"Aunt's Birthday Star Moment closes Pellegrino Peak",
	)
	test.expect(
		shell.handle_player_action(KEYBOARD_SPACE, true),
