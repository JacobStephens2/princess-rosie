...
func _run() -> void:
	...
	test.expect(evidence.get("window_mode") == "fullscreen", "launch starts fullscreen")
	...
	test.expect(
		evidence.get("grown_up_corner_controls") == ["toggle-sound", "replay"],
		"the Grown-up Corner exposes exactly Sound and replay",
	)
