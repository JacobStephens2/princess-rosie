...
ssh -o BatchMode=yes jacob@stephens.page \
  'test "$(readlink -f /var/www/rosi.stephens.page/public)" = /var/www/rosi.stephens.page/public'
```
...
rsync --archive --compress --delete --dry-run --itemize-changes \
  dist/ jacob@stephens.page:/var/www/rosi.stephens.page/public/
```
...
remote_index_sha=$(ssh -o BatchMode=yes jacob@stephens.page \
  'sha256sum /var/www/rosi.stephens.page/public/index.html | cut -d " " -f 1')
...
test "$(curl -sS -o /dev/null -w '%{http_code}' https://rosi.stephens.page/manifest.webmanifest)" = 200
test "$(curl -sS -o /dev/null -w '%{http_code}' https://rosi.stephens.page/sw.js)" = 200
```
...
- Apache configuration: `/etc/apache2/sites-available/rosi.stephens.page.conf`
- Apache HTTPS configuration: `/etc/apache2/sites-available/rosi.stephens.page-le-ssl.conf`
- TLS certificate: Let's Encrypt certificate named `rosi.stephens.page`, renewed automatically by Certbot
