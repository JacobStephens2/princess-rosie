...
### Held back from the Edition Contract

`cue.path-choice.cloister.arches` and `cue.path-choice.cloister.clouds` were generated and
approved while the Cloister still had a Path Choice. #34 deferred Path Choice to post-MVP, so
they have no catalog entry, runtime mapping, or Edition Pack listing. Their masters and
provenance are retained here so the pair can be wired back without paying for them again.
